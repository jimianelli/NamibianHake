...
values of $\gamma, \lambda$ were evaluated and are shown in
...
analysis, we set $\gamma = .25$. For $\lambda$, most models evaluated
were above 0.5 of $B_{MSY}$ so there was no added adjustment beyond the
$R=0.8$. So, following the TAC as specified, the base-case model was
below $B_{MSY}$ so $\delta=0.8$ with the average replacement yield over
the last 5 years is recommended, with the adjustment based on the
relative biomass of *M. paradoxus* and the long-term mean biomass of the
species.
