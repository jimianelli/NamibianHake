...
options(NamibianHake.mods_path = Sys.getenv("NAMIBIANHAKE_MODS_PATH", here::here("mods")))
mods_base <- getOption("NamibianHake.mods_path")
fig_dir <- file.path(mods_base, "figs")
...
ggsave(file.path(fig_dir, "fitidx1.png"), width = 6, height = 5)
```
...
dfcpue |>
  ...
  ggtitle("CPUE 3") +
  ylim(0, NA)
ggsave(file.path(fig_dir, "cpue3.png"), width = 6, height = 5)
```
...
dfcpue |>
  ...
  ggtitle("CPUE 1") +
  ylim(0, NA)
ggsave(file.path(fig_dir, "cpue1.png"), width = 6, height = 5)
```
...
dfcpue |>
  ...
  ggtitle("CPUE 1") +
  ylim(0, NA)
ggsave(file.path(fig_dir, "cpue1_all.png"), width = 6, height = 5)
```
...
dfidx |>
  ...
  geom_line(aes(y = predicted)) +
  geom_point(aes(y = predicted))
ggsave(file.path(fig_dir, "fitidx1_all.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "Age_comp_fish.png"), width = 9, height = 8)
```
...
ggsave(file.path(fig_dir, "Age_comp_surv.png"), width = 9, height = 8)
```
...
p1 <- plot_sel()
# p1
ggsave(file.path(fig_dir, "selectivity.png"), width = 5, height = 9)
```
...
mods |>
  ...
  ylab("SSB") +
  xlab("Year")
ggsave(file.path(fig_dir, "ssb1.png"), width = 6, height = 5)
```
...
mods |>
  ...
  ylab("Relative SSB") +
  xlab("Year")
ggsave(file.path(fig_dir, "depl1.png"), width = 6, height = 5)
```
...
mods |>
  ...
  ylab("Relative SSB") +
  xlab("Year")
ggsave(file.path(fig_dir, "depl2.png"), width = 6, height = 5)
```
...
mods |>
  ...
  xlab("Year")
ggsave(file.path(fig_dir, "rec.png"), width = 6, height = 5)
```
...
mods |>
  ...
  ylab("Recruitment age 0") +
  xlab("Year")
ggsave(file.path(fig_dir, "recalt.png"), width = 6, height = 5)
```
...
p1 <- dfsrr |> ggplot(aes(x = SSB, y = R, color = Model, shape = Model)) +
  ...
  xlim(c(0, 4000))
p1
ggsave(file.path(fig_dir, "srr_curves.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "srr_yrs.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "kobe1.png"), width = 6, height = 5)
```
...
tmp |> ggplot(aes(x = B_Bmsy, label = Year, y = Catch_RY, shape = Model, color = Model, fill = Model)) +
  ...
  xlab("SSB/Bmsy") +
  ylab("Catch / replacement Yield")
ggsave(file.path(fig_dir, "kobe2.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "refpoints.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "biom_paradoxus.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "adj_paradoxus.png"), width = 6, height = 5)
cur_mp <- dftmp |>
...
ggsave(file.path(fig_dir, "gamma.png"), width = 6, height = 5)
```
...
ggsave(file.path(fig_dir, "lambda.png"), width = 6, height = 5)
```
...
prj <- readr::read_csv(
  file.path(mods_base, "m25", "main.out"),
  col_names = FALSE,
