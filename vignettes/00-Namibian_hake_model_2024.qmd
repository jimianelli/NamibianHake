...
p1 <-  biom_out$biomass_by_strata |> filter(strata=="paradoxus") |>
...
dftmp <- p1$data |> select(Year=year, Index = pred) |> mutate(
...
dftmp |> filter(Reactivity != "Index") |>
      ggplot(aes(x = Year, y = Index, color=Reactivity)) + geom_line() +
...
cur_mp <- dftmp |> filter(Year==2025,Reactivity=="Gamma = 0.25") |> pull(Index)
...
df01 |>
  ...
  xlab("Relative M. paradoxus biomass") + ylab("TAC adjustment") +
...
df01 |>  ggplot(aes(y =  adjustment, x=relbiom, color=lambda)) + geom_line() +
  xlab("Spawning biomass relative Bmsy") + ylab("TAC adjustment") +
  ggthemes::theme_few() #+ ylab("TAC adjustment based on M. paradoxus biomass")
