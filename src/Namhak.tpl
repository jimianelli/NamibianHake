...
DATA_SECTION
  ...
  init_int est_Sel_slope //4                     // Selectivity slope parameter
  init_int est_Sel_slopeS //2
  init_int Age_Sel_slopeS //4
  init_int Age_Sel_slope //4                     // Age at which selectivity slope starts
...
!!  NselPar = 2*NSelPeriods;
  ...
  //---------------------------------------------
  init_bounded_number par_B0(-1.0,10.0);                // Virgin biomass
  ...
  sdreport_vector Recstd(first_yr,last_yr)
...
PROCEDURE_SECTION
  ...
  if (sd_phase()) 
  {
   ...
   f = CPUE_Like + Survey_Like + CAA_Likelihood + CAAS_Likelihood + RecRes_Likelihood + weight*Oneyearold_Likelihood + negpen;
...
//    S(Year,0) = 0; 
  ...
  if (select == 0)
  {
    ...
    {
      SurvS(Age) = 1.0/(1+mfexp(-1*TempS*(RageS-SurvA50)));
      if (Age >= Age_Sel_slopeS) SurvS(Age) = SurvS(Age) * mfexp(-SelSlopeS*(Age-Age_Sel_slopeS));
      if (SurvS(Age) > SmaxS) SmaxS = SurvS(Age);
      RageS = RageS + 1;
   }
  }
...
FUNCTION Do_Projections    
  ...
  for (Year=last_yr; Year<=last_yr+NProj; Year++)
  {
   ...
   //cout<<Year<<"  "<<aveRYT<<endl;
      ...
      //cout<<Year<<" 4 "<<futCatch(Year)<<endl;
   ...
   //cout<<Year<<" "<<RevF(Year)<<"  "<<RevW(Year)<<" "<<RevT(Year)<<endl;
    ...
    // -------------------------------------------
    out1<<Year<<" "<<futCatch(Year)<<" "<<Spawn(Year)/mfexp(par_B0)<<" ";
    out2<<Year<<" "<<futCatch(Year)<<" "<<TotalB(Year)/TotalB(1990)<<" ";
    out4<<Year<<"  "<<futCatch(Year)<<" "<<TotalB(Year)/TotalB(1990)<<" "<<Bexp(Year)<<endl;
    out5<<Year<<" "<<futCatch(Year)<<" "<<(Bexp(Year)*qCPU(3))<< " ";
    out6<<Year<<" "<<TAC(Year)<<"  "<<PV(Year)<<" "<<EmployT(Year)<<" " <<ProfitT(Year)<<" "<<VesselF(Year)<<" "<<VesselW(Year)<<" "<<endl;

    out7<<Year<<" "<<futCatch(Year)<<" "<<TotalB(Year)<<" "<<Bmsy<<" "<<TotalB(Year)/Bmsy<<" ";
    out8<<Year<<" "<<futCatch(Year)<<" "<<Spawn(Year)<<" "<<Spmsy<<" "<<Spawn(Year)/Spmsy<<" ";
    out9<<Year<<" "<<futCatch(Year)<<" "<<Spawn(1990)<<" "<<Spmsy<<" "<<Spawn(Year)/Spawn(1990)<<" ";
...
REPORT_SECTION
 ...
 for (Year=1990;Year<=last_yr;Year++)
  ...
  report <<"Model: "<<model_number<<" "<<model_name<<" Weigth= "<<weight<<" select= "<<select<<endl;
  ...
  report << "M                  "<<M(5)<<endl;
  // ---------- Selectivity at age ----------
  report<<"Age S(1968) S(1989) S(2001) survS"<<endl;
  for (Age=0; Age<=plus_grp; Age++)
