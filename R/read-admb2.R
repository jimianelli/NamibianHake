...
read_ctl <- function(fn) {
  options(warn = -1) # Suppress the NA message in the coercion to double
  repfile <- scan(fn,
    what = "character", flush = TRUE, blank.lines.skip = FALSE,
    quiet = TRUE
  )
  idx <- sapply(as.double(repfile), is.na)
  vnam <- repfile[idx] # list names
...
read_dat <- function(fn) {
  ...
  for (i in 1:nv)
  {
    ...
    if (!any(grepl("[a-zA-Z]", dum))) # Logical test to ensure dealing with numbers
      {
        A[[vnam[i]]] <- dum
      } else {
      A[[vnam[i]]] <- as.character(ifile[(ir + 1)]) # Add the item as character strings
    }
  }
  options(warn = 0)
