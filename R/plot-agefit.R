...
case_label="2023 assessment";gear="srv1";type="survey";styr=NULL;ageplus=6
plot_agefit <- function( x, case_label="2023 assessment",gear="srv1",type="survey",styr=NULL,ageplus=NULL) {
   #x=modlst[[thismod]];case_label=af_title;gear="ats";type="survey";styr=1982;ageplus=10
  subtle.color <- "gray40"
  if (gear=="srv1"){
    fage <- 2
    ages <- c(fage,8 ) #age range
    ncols<- length(x$survey_Pobs_1[1,])
    obs.data  <- data.frame(rbind(x$survey_Pobs_1,(matrix(c(2020,rep(0,ncols-1) ), ncol=ncols)) )) |> dplyr::arrange(V1)
    pred.data <- data.frame(rbind(x$survey_Phat_1,(matrix(c(2020,rep(0,ncols-1) ), ncol=ncols)) )) |> dplyr::arrange(V1)
    years     <- obs.data[,1]
  ...
  #q: how do I send the variable value to the console to see its value?
  #a: 
